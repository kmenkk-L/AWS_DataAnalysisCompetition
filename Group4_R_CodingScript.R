...
# VIF(Variance Inflation Factor) of all the variables are similar at value 1, which means the predictor is not correlated with other variables
...
f7 <- glm(formula = Churn ~ TotalCharges + tenure + Fiber.optic + Month.to.month + Electronic.check + 
            ...
            FibMtm , family = binomial(), data = data)
summary(f7)
# All interaction term are significant, but variable TotalCharges is not.
